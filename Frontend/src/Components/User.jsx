...
import {
  User, Award, Shirt, Settings, LogOut, Plus, Eye, Heart, MessageCircle, Clock, CheckCircle, Leaf, TrendingUp, MapPin, Star, Edit3, Banknote
} from 'lucide-react';
...
const UserDashboard = ({ dashboardData, onNavigateToListing, onLogout }) => {
  ...
  const renderOverview = () => (
    <div className="space-y-6">
        <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-2xl p-8 text-white">
            ...
            <p className="text-emerald-100 mb-6">Here's a snapshot of your sustainable journey.</p>
            <div className="grid md:grid-cols-3 gap-4">
                <div className="bg-white/10 rounded-xl p-4"><div className="text-2xl font-bold">{profile.points}</div><div className="text-emerald-100 text-sm">Available Points</div></div>
                <div className="bg-white/10 rounded-xl p-4"><div className="text-2xl font-bold">{itemsOverview.listed}</div><div className="text-emerald-100 text-sm">Active Listings</div></div>
                <div className="bg-white/10 rounded-xl p-4"><div className="text-2xl font-bold">{ongoingSwaps.length}</div><div className="text-emerald-100 text-sm">Ongoing Swaps</div></div>
            </div>
        </div>
  ...
  const renderItems = () => (
    <div className="space-y-6">
        <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-gray-900">My Items ({userItems.length})</h2>
  ...
  return (
    // The main container div should not have sticky positioning
    <div className="bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid lg:grid-cols-4 gap-8">
          {/* Sidebar */}
          <div className="lg:col-span-1 lg:sticky lg:top-24 self-start"> {/* Making sidebar sticky */}
              ...
              <div className="space-y-2 mb-6 text-center">
                  <div className="bg-emerald-50 rounded-lg p-3">
                   ...
                   <div className="bg-gray-50 rounded-lg p-3">
                    <div className="text-lg font-bold text-gray-900">{itemsOverview.listed}</div>
            ...
            <nav className="bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
              <div className="space-y-2">
                {[
                    {name: 'overview', icon: TrendingUp, label: 'Overview'},
                    {name: 'items', icon: Shirt, label: 'My Items'},
                    {name: 'transactions', icon: Banknote, label: 'Transactions'},
